...
const CatBannerList = ({navigation}) => {

    const banner = useSelector(x=>x.banner.catList)
    const dispatch = useDispatch()
    console.log('Cat bnee', banner)

    const deletehandler = async(id) => {
        await dispatch(bannerAction.deleteCatBanner(id))
        dispatch(bannerAction.fetchCatBanner())
    }
    ...
    return(
        ...
        <Pressable
            ...
            onPress={()=>navigation.navigate('CategoryBanner',{item:[], type:'Create'})}
        />
        </SafeAreaView>
    )
